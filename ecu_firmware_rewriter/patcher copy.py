...
load_at = 0x08000000
CAN_TRANSMIT_TIMEOUT = 0x08048d91
CAN_DRIVER = 0x200025e8
ECUID=0x12345678
NONCE=0xabcdef

o = 0x802C504 - 0x802c4d0 #52
rewriting_point = 0x802aa41 + o #0x802aa75
...
def gen_detour(from_offset, func_offset, detour_offset):
    ...
    detour_code += ArmUtils.movt(1, 0x2000)
    pc += 4
    detour_code += ArmUtils.mov(1, 0x15fc)
    pc += 4
    detour_code += ArmUtils.store(ArmUtils.SP, 1, 0)
    pc += 4
    detour_code += ArmUtils.movt(ArmUtils.SP, 0x2000)
    pc += 4
    detour_code += ArmUtils.mov(ArmUtils.SP, 0x15fc)
    pc += 4
    ...
    detour_code = ArmUtils.call(func_offset - detour_offset)
    pc += 4
